...
VOID c_overlay::init_imgui() {
	...
	style.Colors[ImGuiCol_TitleBg] = ImColor(38, 31, 45, 255);
	style.Colors[ImGuiCol_TitleBgCollapsed] = ImColor(38, 31, 40, 255);
	style.Colors[ImGuiCol_TitleBgActive] = ImColor(38, 31, 45, 255);
...
const void c_overlay::render(FLOAT width, FLOAT height) {
		...
		for (playerIndex < 32; ++playerIndex;) {
			...
			if (feetPos.z >= 0.5f) {
				...
				if (config::esp::bHealthBar) {
					...
					ImVec2 healthBarMax = { screenHead.x - width - 5.7f, TrueScreenCock.y };
	...
	ImGui::Begin("Idealdriver | for free use", nullptr, ImGuiWindowFlags_NoResize);
		...
		ImGui::SameLine();
			if (gui::showVisuals) {
				...
				if (config::esp::bEnable) {
					...
					if (config::esp::bBoxes) {
						...
						ImGui::SliderFloat("Box rounding", &config::esp::bRounding, 1.0f, 80.0f); //HOLD ON TIME
					}*/
					ImGui::Text("Bone");
					ImGui::Separator();
					...
					//ImGui::Checkbox("Health Bar", &config::esp::bHealthBar); //HOLD ON TIME
			...
			if (gui::showAim) {
				...
				if (config::aim::bAimbot) {
					...
					//ImGui::Checkbox("Autofire on aim radius", &config::aim::bAutofire); Soon
					ImGui::Text("Smooth");
					ImGui::SameLine();
			...
			if (gui::showMisc) {
				...
				//ImGui::Checkbox("Radarhack", &config::esp::bRadar); //HOLD ON TIME (SPOOTED DETECT)
				//::Checkbox("Imgui demo", &config::esp::bDemo); //FOR TEST
