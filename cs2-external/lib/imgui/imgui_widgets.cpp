...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), 0.0f, 0, border_size);

    // Render button
    RenderFrame(bb.Min + ImVec2(border_size, border_size), bb.Max - ImVec2(border_size, border_size), col, true, style.FrameRounding);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(border_size, border_size), bb.Max - style.FramePadding - ImVec2(border_size, border_size), label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::ButtonNoBg(const char* label, const ImVec2& size_arg)
{
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float checkbox_height = 15.0f;
    const float checkbox_width = 15.0f;
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(checkbox_width, checkbox_height));
    ItemSize(check_bb, style.FramePadding.y);
    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos + ImVec2(0, style.FramePadding.y), window->DC.CursorPos + ImVec2(0, style.FramePadding.y) + ImVec2(checkbox_width, checkbox_height));
    if (label_size.x > 0)
    {
    ...
    const float pad = ImMax(0.0f, (float)(int)(check_sz / 3.5f));

    // Draw black border
    const float border_size = 3.2f; // Установите желаемый размер обводки
    window->DrawList->AddRect(check_bb.Min + ImVec2(border_size, border_size), check_bb.Max - ImVec2(border_size, border_size), IM_COL32(0, 0, 0, 255));
    ...
    {
        auto color_black_off = ImGui::GetColorU32(ImVec4(0.2, 0.2, 0.2, 0.80));
        auto color_grey_off = ImGui::GetColorU32(ImVec4(0.2, 0.2, 0.2, 0.40));

        //off
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
        window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), color_grey_off, color_grey_off, color_black_off, color_black_off);
    }
    if (label_size.x > 0.0f)
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    ImVec2 modified_frame_bb_max = ImVec2(frame_bb.Max.x, frame_bb.Max.y - 9);
    ImVec2 modified_frame_bb_min = ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9);
    RenderFrame(modified_frame_bb_min, modified_frame_bb_max, frame_col, true, 10);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(modified_frame_bb_min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 10, style.GrabRounding); // fricked up here
        grab_bb.Max.y -= 2;
        grab_bb.Max.x -= 6;
        window->DrawList->AddCircleFilled(grab_bb.Max, 6, ImColor(255, 255, 255), 14);
    }
